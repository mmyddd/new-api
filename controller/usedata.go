...
func GetAllFlowQuotaDates(c *gin.Context) {
	...
	username := c.Query("username")
	tokenName := c.Query("token_name")
	dates, err := model.GetFlowQuotaData(startTimestamp, endTimestamp, username, tokenName, 0, c.GetInt("role"))
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetUserFlowQuotaDates(c *gin.Context) {
	...
	tokenName := c.Query("token_name")
	dates, err := model.GetFlowQuotaData(startTimestamp, endTimestamp, "", tokenName, userId, common.RoleCommonUser)
	if err != nil {
		common.ApiError(c, err)
		return
	}
